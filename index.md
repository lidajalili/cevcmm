...
random-effects vector $`\alpha`$ is assumed to covary — **not** what
$`Z`$’s entries look like. The distribution of $`Z`$ (binary indicators,
...
- **[`vignette("getting-started", package = "cevcmm")`](https://lidajalili.github.io/cevcmm/articles/getting-started.md)**
  — single-machine fit on a simple diagonal-random-effects example.
  Walks through every public S3 method.
- **[`vignette("distributed-fitting", package = "cevcmm")`](https://lidajalili.github.io/cevcmm/articles/distributed-fitting.md)**
  — split data across $`K`$ nodes, compute per-node summaries with
  [`node_summary()`](https://lidajalili.github.io/cevcmm/reference/node_summary.md),
  and recover a fit bit-equivalent to pooled
  [`vcmm()`](https://lidajalili.github.io/cevcmm/reference/vcmm.md) via
  [`fit_from_summaries()`](https://lidajalili.github.io/cevcmm/reference/fit_from_summaries.md).
  Also covers the streaming-accumulator pattern for memory-constrained
  settings.
- **[`vignette("od-migration", package = "cevcmm")`](https://lidajalili.github.io/cevcmm/articles/od-migration.md)**
  — Kronecker covariance for origin-destination flow data, using a
  bundled simulated migration dataset.
...
GPL-3. See the [`LICENSE`](https://lidajalili.github.io/cevcmm/LICENSE)
