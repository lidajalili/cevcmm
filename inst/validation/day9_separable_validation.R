...
pass_A <- c(
  ...
  A_beta_0_unbiased = {
    ## Adaptive criterion: accept the bias if EITHER it's within 2 sigma of
    ## the observed MC SE on the mean, OR it is below an absolute floor of
    ## 0.05. The first arm handles indicator-Z designs (large per-rep SD
    ## because identifiability post-processing absorbs 2*mean(alpha) into
    ## beta_0); the floor handles dense-Z designs (per-rep SD is tiny and
    ## the 2-sigma test would otherwise reject sub-percent finite-sample
    ## spline noise).
    mc_se_A <- sd(beta0_hats_A) / sqrt(M_A)
    thr_A   <- max(2 * mc_se_A, 0.05)
    cat(sprintf("[A] beta_0 per-rep SD = %.4f, MC SE on mean = %.4f, threshold = max(2*SE, 0.05) = %.4f\n",
                sd(beta0_hats_A), mc_se_A, thr_A))
    abs(mean(beta0_hats_A) - beta0_A) < thr_A
  }
)
...
pass_B <- c(
  ...
  B_beta_0_unbiased = {
    ...
    thr_B   <- max(2 * mc_se_B, 0.05)
    cat(sprintf("[B] beta_0 per-rep SD = %.4f, MC SE on mean = %.4f, threshold = max(2*SE, 0.05) = %.4f\n",
                sd(beta0_hats_B), mc_se_B, thr_B))
    abs(mean(beta0_hats_B) - beta0_B) < thr_B
  }
)
